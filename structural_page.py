...
models = [m for m in project_data.models.items if m.name.startswith('structure')]
...
def plot_bar_chart(value1, value2):
    df = pd.DataFrame({
        'category': ['Standard wind loads', 'Hyper Team B Building wind loads'],
        'values': [value1, value2]
    })
    
    fig = px.bar(df, x='category', y='values', title="Building Wind Loads Analysis", 
...
def plot_bar_chart2(value1, value2):
    df = pd.DataFrame({
        'category': ['Standard bridge load transfer', 'Hyper Team B Building bridge load transfer'],
        'values': [value1, value2]
    })
    custom_colors = ['#1f77b4', '#ff7f0e']  # Blue and orange
    fig = px.bar(df, x='category', y='values', title="Bridge Load Transfer Analysis", 
...
with gr.Blocks() as s_demo:
    with gr.Row():
        model_dropdown = gr.Dropdown(choices=models_name, label="Select Structural Team Model")
