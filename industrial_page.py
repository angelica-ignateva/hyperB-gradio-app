...
models = [m for m in project_data.models.items if m.name.startswith('industrial')]
...
def plot_pie_chart(value1, value2):
    df = pd.DataFrame({
        'category': ['Local food', 'Other sources'],
        'values': [value1, value2]
    })
    fig = px.pie(df, values='values', names='category', title="Residents diet")
...
def plot_pie_chart2(value1, value2):
    df = pd.DataFrame({
        'category': ['Income generated cost coverage', 'Residents-paid costs'],
        'values': [value1, value2]
    })
    custom_colors = ['#1f77b4', '#ff7f0e']  # Blue and orange
    fig = px.pie(df, values='values', names='category', title="Residents costs", color_discrete_sequence=custom_colors)
...
with gr.Blocks() as i_demo:
    with gr.Row():
        model_dropdown = gr.Dropdown(choices=models_name, label="Select Industrial Team Model")
    ...
    with gr.Row():
        with gr.Column():
            value1 = gr.Number(label="Local food resources (%)", value=53)
            value2 = gr.Number(label="Other food resources (%)", value=47)
            submit_btn1 = gr.Button("Submit")
        
        with gr.Column():
            value3 = gr.Number(label="Income generated cost coverage (%)", value=60)
            value4 = gr.Number(label="Residents-paid costs (%)", value=40)
    ...
    submit_btn2.click(plot_pie_chart2, inputs=[value3, value4], outputs=output2)

    # Automatically generate the plot when the app loads
    i_demo.load(plot_pie_chart, inputs=[value1, value2], outputs=output1)
    i_demo.load(plot_pie_chart2, inputs=[value3, value4], outputs=output2)
