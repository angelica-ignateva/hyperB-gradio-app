...
models = [m for m in project_data.models.items if m.name.startswith('facade')]
...
def load_default_model_and_version(project_data):
    ...
    first_model = models[1].name
    versions = client.version.get_versions(model_id=models[1].id, project_id=project_data.id, limit=100).items
    if not versions:
...
def plot_pie_chart(value1, value2):
    ...
    df = pd.DataFrame({
        'category': ['Facade-generated energy', 'Remaining nergy demand'],
        'values': [value1, value3]
    })
    
    fig = px.pie(df, values='values', names='category', title="Facade-generated energy ratio")
...
def plot_bar_chart2(value1, value2):
    df = pd.DataFrame({
        'category': ['Standard air-quality', 'Improved air-quality'],
        'values': [value1, value2]
    })
    custom_colors = ['#1f77b4', '#ff7f0e']  # Blue and orange
    fig = px.bar(df, x='category', y='values', title="Air quality index", 
...
with gr.Blocks() as f_demo:
    with gr.Row():
        model_dropdown = gr.Dropdown(choices=models_name, label="Select Facade Team Model")
    ...
    with gr.Row():
        with gr.Column():
            value1 = gr.Number(label="Facade-generated energy (%)", value=45)
            value2 = gr.Number(label="Energy demand (%)", value=100)
            submit_btn1 = gr.Button("Submit")
        
        with gr.Column():
            value3 = gr.Number(label="Standard air-quality", value=100)
            value4 = gr.Number(label="Improved air-quality", value=40)
    ...
    submit_btn1.click(plot_pie_chart, inputs=[value1, value2], outputs=output1)
    submit_btn2.click(plot_bar_chart2, inputs=[value3, value4], outputs=output2)

    # Automatically generate the plot when the app loads
    f_demo.load(plot_pie_chart, inputs=[value1, value2], outputs=output1)
    f_demo.load(plot_bar_chart2, inputs=[value3, value4], outputs=output2)
